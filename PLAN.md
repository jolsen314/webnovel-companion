...
> **[WP-62](docs/PLAN-archive.md#wp-62--skip-docs-only-deployments-vercel-ignored-build-step-done-2026-09-07)
> shipped 2026-09-07** — docs-only commits no longer deploy (`vercel.json` `ignoreCommand`), which matters because
...
| WP-61 | *(low — sibling to WP-31)* **Serverless-render hydration for interaction-gated captures** — surfaced live-testing WP-54's `probe-api`: some client SPAs that hydrate in a normal browser **don't execute their client JS on Vercel's `@sparticuz/chromium`** (the rendered SSR HTML is full and contains the control, but the app's own on-load XHRs never fire — only third-party scripts do), so a chapter-list request gated behind hydration + interaction can't be captured *there* even though the detector + nudge work locally end-to-end. Not gating (page renders 200, no CF challenge) and not the SSRF guard (mirrored locally, still hydrates). Options to probe: longer settle / `waitForFunction` on a hydration signal / different chromium flags / a hydration-wait before the nudge. Affects `probe-api` capture + any future render-interaction (WP-31). **Prerequisite for WP-31 on its non-hydrating subset:** all of WP-31's interactions (tabs/pagination/scroll/RSC) share this single hydration prerequisite, so where a WP-31 driver doesn't hydrate on the serverless renderer, WP-31 is dead-on-arrival for it until WP-61 is solved (or it moves to residential-ingest WP-60). The API path (WP-45 `renderJson`) is unaffected — it never runs the app. Workaround today: read the endpoint by hand + wire via `set-api-descriptor` (the API fetch itself is unaffected). **Drivers (2026-09-03 probe) — two hydration-failure shapes:** **B08** (SSR HTML is *full* incl. the control, but the app's on-load/interaction XHRs never fire on the deployed render — 589-ch list captured locally, 0 app XHRs deployed) and **B05** (a *pure CSR* SPA — the served HTML is a ~4KB shell with 0 chapters, so nothing renders at all without hydration). **Separate robustness gap surfaced by B05:** its `probe-api` capture *hung the render to the function ceiling* despite WP-54 (#36)'s per-op bounds — an unbounded render op (likely the SSRF-interception DNS wait) still isn't capped; an **overall capture deadline** would guarantee termination. Deferred. | `TODO` | WP-17b, WP-54 |
...
WP-62 (skip docs-only deployments — `vercel.json` `ignoreCommand`; a skipped build stores 0 MB. Also deleted `vercel.json`'s inert `functions` block and filed WP-63/WP-64).
