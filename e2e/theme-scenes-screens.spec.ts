...
test.describe('main pass — asset base set (NEXT_PUBLIC_THEME_ASSET_BASE=/themes)', () => {
  ...
  for (const theme of THEMES) {
    test(`empty hero / — ${theme}`, async ({ page }) => {
      ...
      await page.screenshot({ path: `${OUT}/${theme}-hero.png`, fullPage: true });
    });

    test(`populated shelf / — ${theme}`, async ({ page }) => {
      ...
      await page.screenshot({ path: `${OUT}/${theme}-shelf.png`, fullPage: true });
    });
    ...
    test(`login — ${theme}`, async ({ page }) => {
      ...
      await page.screenshot({ path: `${OUT}/${theme}-login.png`, fullPage: true });
    });

    test(`series detail — ${theme}`, async ({ page }) => {
      ...
      await page.screenshot({ path: `${OUT}/${theme}-detail.png`, fullPage: true });
    });
    ...
    test(`add — ${theme}`, async ({ page }) => {
      ...
      await page.screenshot({ path: `${OUT}/${theme}-add.png`, fullPage: true });
    });
  }
  ...
  test('night empty hero / — hero de-emphasis sign-off', async ({ page }) => {
    await setTheme(page, 'night');
    await page.goto('/');
    await expect(page.locator('html')).toHaveAttribute('data-theme', 'night');
    await expect(page.getByRole('heading', { name: /quiet in here/i })).toBeVisible();
    await page.screenshot({ path: `${OUT}/night-hero.png`, fullPage: true });
  });
});
...
test.describe('fallback pass — asset base unset/unreachable', () => {
  ...
  test('scroll hero — no tree image (graceful degradation)', async ({ page }) => {
    ...
    await page.screenshot({ path: `${OUT}/scroll-hero-fallback.png`, fullPage: true });
  });

  test('scroll card — red-circle badge, not wax seal (graceful degradation)', async ({ page }) => {
    await seedSeries({
    ...
    await page.screenshot({ path: `${OUT}/scroll-card-fallback.png`, fullPage: true });
  });
});
